...
public class ProcessingHelper {
    ...
    public static <T extends IntegerType<T> & NativeType<T>> int getSurfaceAreaContributionOfVoxelInFaces(
	    final RandomAccess<T> ra, long[] offset, long[] dimensions, boolean isSegmentation) {
	...
	for (long[] currentVoxel : voxelsToCheckForSurface) {
	    final long currentPosition[] = { pos[0] + currentVoxel[0], pos[1] + currentVoxel[1],
		    pos[2] + currentVoxel[2] };
	    final long currentGlobalPosition[] = { currentPosition[0] + offset[0], currentPosition[1] + offset[1],
	    ...
	    if (isSurfaceFace && (currentGlobalPosition[0] >= 0 && currentGlobalPosition[1] >= 0
		    && currentGlobalPosition[2] >= 0 && currentGlobalPosition[0] < dimensions[0]
		    && currentGlobalPosition[1] < dimensions[1] && currentGlobalPosition[2] < dimensions[2])) {
	...
	Long ID = dimensions[0] * dimensions[1] * position[2] + dimensions[0] * position[1] + position[0] + 1;
	return ID;
    }
    ...
    public static DataType createDatasetUsingTemplateDataset(String templateN5Path, String templateDatasetName,
	    String newN5Path, String newDatasetName) throws IOException {
	return createDatasetUsingTemplateDataset(templateN5Path, templateDatasetName, newN5Path, newDatasetName, null);
    }

    public static DataType createDatasetUsingTemplateDataset(String templateN5Path, String templateDatasetName,
	    String newN5Path, String newDatasetName, DataType dataType, int[] blockSize) throws IOException {
	...
	n5Writer.createDataset(newDatasetName, dimensions, blockSize,
		dataType == null ? attributes.getDataType() : dataType, new GzipCompression());
    ...
    public static DataType createDatasetUsingTemplateDataset(String templateN5Path, String templateDatasetName,
	    String newN5Path, String newDatasetName, DataType dataType) throws IOException {
	final N5Reader n5Reader = N5GenericReader(templateN5Path);
	final DatasetAttributes attributes = n5Reader.getDatasetAttributes(templateDatasetName);
	final long[] dimensions = attributes.getDimensions();
    ...
    public static <T extends NumericType<T>> IntervalView<T> getOffsetIntervalExtendZeroRAI(String n5Path,
	    String dataset, long[] offset, long[] dimension) throws IOException {
	final N5Reader n5Reader = N5GenericReader(n5Path);
	if (Files.exists(Paths.get(n5Path + "/" + dataset + "/s0"))) { // in case is multiscale
	    dataset = dataset + "/s0";
	}
	N5Utils.open(n5Reader, dataset);
	return Views.offsetInterval(Views.extendZero((RandomAccessibleInterval<T>) N5Utils.open(n5Reader, dataset)),
		offset, dimension);
    }

    public static <T extends NumericType<T>> IntervalView<T> getZerosIntegerImageRAI(long[] dimension,
	    DataType dataType) {

	if (dataType == DataType.UINT8) {
	    return (IntervalView<T>) Views.offsetInterval(ArrayImgs.unsignedBytes(dimension), new long[] { 0, 0, 0 },
		    dimension);
	} else if (dataType == DataType.UINT16) {
	    return (IntervalView<T>) Views.offsetInterval(ArrayImgs.unsignedShorts(dimension), new long[] { 0, 0, 0 },
		    dimension);

	} else if (dataType == DataType.UINT32) {
	    return (IntervalView<T>) Views.offsetInterval(ArrayImgs.unsignedInts(dimension), new long[] { 0, 0, 0 },
		    dimension);

	} else if (dataType == DataType.UINT64) {
	    return (IntervalView<T>) Views.offsetInterval(ArrayImgs.unsignedLongs(dimension), new long[] { 0, 0, 0 },
		    dimension);
	}
	return null;
    }
	...
	return (IntervalView<NativeBoolType>) Views.offsetInterval(ArrayImgs.booleans(dimension),
		new long[] { 0, 0, 0 }, dimension);

    }

    public static <T extends NumericType<T>> RandomAccess<T> getOffsetIntervalExtendZeroRA(String n5Path,
	    String dataset, long[] offset, long[] dimension) throws IOException {
	IntervalView<T> rai = getOffsetIntervalExtendZeroRAI(n5Path, dataset, offset, dimension);
	return rai.randomAccess();
    }

    public static <T extends NumericType<T>> Cursor<T> getOffsetIntervalExtendZeroC(String n5Path, String dataset,
	    long[] offset, long[] dimension) throws IOException {
	IntervalView<T> rai = getOffsetIntervalExtendZeroRAI(n5Path, dataset, offset, dimension);
	return rai.cursor();
    }
